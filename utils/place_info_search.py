...
class GooglePlaceSearchTool:
    ...
    def google_search_transportation(self, place:str) -> dict:
        """
        Searches for avaiable modes of transportation in the specified place using GooglePlaces API. 
        """
        return self.places_tool.run(f"What are the difference modes of transportations available in {place} ?")
...
class TavilyPlaceSearchTool:
    ...
    def tavily_search_restaurants(self, place: str) -> dict:
        ...
        """
        tavily_tool = TavilySearch(topic="general", include_answer="advanced")
        result = tavily_tool.invoke({"query":f"What are the top 10 restuarants and eateries in and around {place}"})
        if isinstance(result, dict) and result.get("answer"):
            return result["answer"]
        return result
    ...
    def tavily_search_activity(self, place:str) -> dict:
        ...
        if isinstance() and result.get("answer"):
            return result["answer"]
        return result
